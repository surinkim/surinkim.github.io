...
Weekly posts are generated by the `weekly-digest` skill (`.claude/skills/weekly-digest/SKILL.md`). The post has 3 sections: 📰 테크 뉴스 (10), 🚀 Indie Radar (5~7), 📚 도서 (알라딘 소설/IT/인문).

**Candidate collection (current):**
```bash
# Collect candidates + popularity signals for all 3 sections (~30s, no history side effects)
...
```
- Output: `jobs/data/runs/<to>/radar_{news,indie,books}.md` + `radar_candidates.json`
